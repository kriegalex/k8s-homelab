...
## How to release PVs created with Retain reclaim policy ?

```console
kubectl patch pv $PV -p '{"spec":{"claimRef": null}}'
```

This is needed if you uninstall an Helm chart, then install it again, as the PV previously created will be in `Released` state. This means it won't be available to bind to new claims.
